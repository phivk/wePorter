<?php
  ...
  */
  ...
  $db_servername = "localhost";
  $db_username = "ma101pvk";
  $db_password = "conrrite";
  $db_name = "ma101pvk_weporter";
  ...
  "Please fill in a valid email aformddress");
      ...
      {        
        ...
        $formData['pleasant'] = intval($mysqli->real_escape_string($_POST['formPleasant']));
        $formData['informative'] = intval($mysqli->real_escape_string($_POST['formInformative']));

        // strings
        $formData['focus'] = $mysqli->real_escape_string(nl2br(strip_tags($_POST['formFocus'])));
        ...
        $insertQuery = "INSERT INTO Questionnaires (`created`,`formName`,`formEmail`,`formAge`,`formHear`,`formFrequency`,`formDuration`,`formNews`,`formRecord`,`formUpload`,`formRemix`,`formPleasant`,`formInformative`,`formFocus`,`formComments`)
                  VALUES
                  (
                  NOW(), '".
                  $formData['name']."', '".
